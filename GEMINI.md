...
- **Izumi: `mediagent_config.json` is only packaged from `demos/backend/`.** The deploy script copies `demos/backend/*` and `mediagent_kit/` into the bundle and nothing else, so upstream's repo-root copy never reaches the deployed agent — it only affects local runs. Write it to `demos/backend/mediagent_config.json`; those items land at the bundle root (`/code`), which is where `MediagentKitConfig` looks (both cwd and the parent of `mediagent_kit/`).
- **Izumi: every patch must be verified, never silent.** A `sed` that stops matching after an upstream refactor would either break the agent or silently violate data residency. Always `grep` first and report success/failure explicitly.
- **Izumi model pinning.** `gemini-2.5-flash` and `gemini-2.5-pro` are in upstream's `_compatible_models` for text and are both available on `europe-west1`/`europe-west4`. Only the `text` family is overridden in JSON; `MediagentKitConfig` merges per-family with `.update()`, so image/video/music/tts keep their defaults.
...
- **`IZUMI_REGIONAL_MEDIA` is opt-in on purpose.** Pinning media to older models (`gemini-2.5-flash-image`, `imagen-4.0-generate-001`, `veo-3.0-generate-001`, `lyria-002`, `gemini-2.5-flash-tts`) costs output quality, and the newer defaults work today. Only set it for clients whose Vertex processing must stay in-region. Text pinning, by contrast, is always on — it is mandatory once the endpoint is regional.
